...
O Log Pose e o app companion do Only OS (Electron + React). O **playbook** (`d:\code\win\only-os-playbook`)
...
ja expoe `setTweak`, `tweakStatus`, `listStartup`, `setStartup` (`electron/preload.js`, backend em
`electron/main.js`). A tarefa e **estender** esse padrao, nao criar do zero.
...
1. **Investigar o backend** (`electron/main.js`): achar como `setTweak`/`tweakStatus`/`setStartup`/
...
- Testar: build do app + rodar e alternar um toggle de ida e volta, conferindo `services.msc`.
