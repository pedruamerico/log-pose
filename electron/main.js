...
// De-elevation for installers that refuse an admin context (winget error
// 0x8A150056 / exit -1978335146, e.g. Spotify). Log Pose runs elevated
// (requireAdministrator), so we relaunch the winget command as the interactive
// (non-elevated) user via a transient scheduled task: register it with a Limited
// run level under the current user, run it, poll until done, read LastTaskResult,
// then delete the task. Streaming isn't available through schtasks, so we emit a
// notice and the final exit code only.
const ERR_PROHIBITS_ELEVATION = -1978335146;

function runDeelevated(onLine, wingetExe, wingetArgs) {
    return new Promise((resolve) => {
        const taskName = 'LogPoseDeelevate';
        // Quote each winget arg; the task action runs winget.exe directly.
        const quoted = wingetArgs.map(a => `'${a.replace(/'/g, "''")}'`).join(', ');
        // PS: create a Limited-runlevel task as the logged-on user, start it, wait,
        // capture the result, then remove it. Always clean up (finally-style).
        const ps = `
$ErrorActionPreference='Stop'
$exe='${wingetExe.replace(/'/g, "''")}'
$argList=@(${quoted})
...
try {
  ...
  Unregister-ScheduledTask -TaskName '${taskName}' -Confirm:$false -ErrorAction SilentlyContinue
        ...
        onLine?.('==> O instalador recusou o modo administrador; reinstalando como usuário comum...\n');
        ...
        child.on('close', () => {
            const m = buf.match(/RESULT=(-?\d+)/);
            const code = m ? parseInt(m[1], 10) : -1;
            onLine?.(code === 0
                ? '==> Instalação concluída (modo usuário).\n'
                : `==> Falhou no modo usuário (código ${code}).\n`);
            resolve(code);
        });
        child.on('error', (err) => { onLine?.(`error: ${err.message}\n`); resolve(-1); });
    });
}
...
ipcMain.handle('app:download-run', async (event, payload) => {
    ...
    try {
        ...
        fs.closeSync(fd);
        if (stat.size < 1024 || head.toString('latin1') !== 'MZ') {
            try { fs.rmSync(dest, { force: true }); } catch {}
            throw new Error(`o servidor não retornou um instalador válido (${stat.size} bytes — não é um .exe do Windows)`);
        }
...
// `source` is optional ("msstore" for Store-only apps like WhatsApp). The
// renderer sends { packageId, source }.
ipcMain.handle('winget:install', async (event, payload) => {
    const packageId = typeof payload === 'string' ? payload : payload?.packageId;
    ...
    const channelOut = `winget:out:${packageId}`;
    const out = (s) => event.sender.send(channelOut, s);
    const isMsstore = source === 'msstore';
    ...
    if (source === 'winget' || source === 'msstore') { args.push('--source', source); }

    const wingetExe = resolveWinget();
    ...
    // Store apps need the Microsoft Store / its winget source present. On Atlas
    // OS and similar de-bloated images the Store is often removed, so the install
    // fails — surface a clear, actionable message.
    const needsStore = code !== 0 && isMsstore;
    if (needsStore) {
    ...
    return { ok: code === 0, exitCode: code, needsStore };
});
...
const NAME_FALLBACK = {
    ...
    // WhatsApp ships only as a Microsoft Store app; match by name in case
    ...
    'AmdAdrenalin':                ['AMD Software', 'Adrenalin'],
};
...
const COPILOT = 'HKLM\\SOFTWARE\\Policies\\Microsoft\\Windows\\WindowsCopilot';
const WINAI = 'HKLM\\SOFTWARE\\Policies\\Microsoft\\Windows\\WindowsAI';
const EXPLORER_POL = 'HKCU\\SOFTWARE\\Policies\\Microsoft\\Windows\\Explorer';
...
const TWEAKS = {
    ...
    // AI / Windows policies. These toggles are "disable X": enabled=true applies
    // the disabling policy, and status reports "on" when that policy is present.
    'copilot-off': {
        on:  [regAdd(COPILOT, 'TurnOffWindowsCopilot', 'REG_DWORD', 1)],
        off: [regDel(COPILOT, 'TurnOffWindowsCopilot')],
        status: { path: COPILOT, name: 'TurnOffWindowsCopilot', onValue: '0x1' }
    },
    'recall-off': {
        on:  [regAdd(WINAI, 'DisableAIDataAnalysis', 'REG_DWORD', 1),
              regAdd(WINAI, 'AllowRecallEnablement', 'REG_DWORD', 0)],
        off: [regDel(WINAI, 'DisableAIDataAnalysis'), regDel(WINAI, 'AllowRecallEnablement')],
        status: { path: WINAI, name: 'DisableAIDataAnalysis', onValue: '0x1' }
    },
    'ai-cocreator-off': {
        on:  [regAdd(WINAI, 'DisableImageCreator', 'REG_DWORD', 1),
              regAdd(WINAI, 'DisableCocreator', 'REG_DWORD', 1),
              regAdd(WINAI, 'DisableGenerativeFill', 'REG_DWORD', 1),
              regAdd(WINAI, 'DisableImageGenerativeErase', 'REG_DWORD', 1)],
        off: [regDel(WINAI, 'DisableImageCreator'), regDel(WINAI, 'DisableCocreator'),
              regDel(WINAI, 'DisableGenerativeFill'), regDel(WINAI, 'DisableImageGenerativeErase')],
        status: { path: WINAI, name: 'DisableCocreator', onValue: '0x1' }
    },
    'web-search-off': {
        on:  [regAdd(EXPLORER_POL, 'DisableSearchBoxSuggestions', 'REG_DWORD', 1)],
        off: [regDel(EXPLORER_POL, 'DisableSearchBoxSuggestions')],
        status: { path: EXPLORER_POL, name: 'DisableSearchBoxSuggestions', onValue: '0x1' }
    }
};
...
// --- IPC: winget show (live details for the info popover) -------------------
// winget localizes the field labels, so we match English + pt-BR spellings.
ipcMain.handle('winget:show', async (event, packageId) => {
    if (!/^[A-Za-z0-9._+\-]+$/.test(packageId || '')) return { ok: false };
    return new Promise((resolve) => {
        execFile(resolveWinget(),
            ['show', '--id', packageId, '--exact', '--disable-interactivity', '--accept-source-agreements'],
            { windowsHide: true, maxBuffer: 10 * 1024 * 1024 },
            (err, stdout) => {
                if (!stdout) { resolve({ ok: false }); return; }
                const clean = stdout.replace(/\x1b\[[0-9;?]*[A-Za-z]/g, '').replace(/[\r\b]/g, '\n');
                const field = (labels) => {
                    for (const line of clean.split('\n')) {
                        const l = line.trimStart();
                        for (const lab of labels) {
                            if (l.startsWith(lab)) {
                                const v = l.slice(lab.length).trim();
                                if (v) return v;
                            }
                        }
                    }
                ...
                resolve({
                    ok: true,
                    version:     field(['Version:', 'Versão:']),
                    publisher:   field(['Publisher:', 'Editor:']),
                    description: field(['Description:', 'Descrição:']),
                    homepage:    field(['Homepage:', 'Página Inicial:', 'Página inicial:']),
                });
            });
    });
});

// --- IPC: AppX debloat (live present/remove/restore) ------------------------
ipcMain.handle('appx:list', async () => {
    const out = psQuery('Get-AppxPackage | ForEach-Object { $_.Name }');
    const installed = out ? out.split(/\r?\n/).map(s => s.trim()).filter(Boolean) : [];
    return { ok: !!out, installed };
});

// Slow (~10-30s): installed Windows capabilities (Recall etc.). Called off the
// boot path. Returns short names (before the first '~').
ipcMain.handle('appx:capabilities', async () => {
    const out = psQuery(
        "Get-WindowsCapability -Online | Where-Object State -eq 'Installed' | ForEach-Object { ($_.Name -split '~')[0] }");
    const installed = out ? [...new Set(out.split(/\r?\n/).map(s => s.trim()).filter(Boolean))] : [];
    return { ok: !!out, installed };
});

// Remove both the per-user AND provisioned copies so the app doesn't reinstall
// on new accounts.
ipcMain.handle('appx:remove', async (event, name) => {
    ...
    const ps =
        ...
        `Remove-AppxProvisionedPackage -Online -ErrorAction SilentlyContinue; Write-Output 'done'`;
    const code = await streamProcess(event, `appx:out:${name}`, `appx:done:${name}`,
        'powershell.exe', ['-NoProfile', '-NonInteractive', '-ExecutionPolicy', 'Bypass', '-Command', ps]);
    return { ok: code === 0, exitCode: code };
});

// Re-register a staged copy for all users, or open the Store page if none.
ipcMain.handle('appx:restore', async (event, name) => {
    if (!/^[A-Za-z0-9._*\-]+$/.test(name || '')) return { ok: false, error: 'invalid name' };
    const ps =
        `$pkg = Get-AppxPackage -AllUsers '${name}' | Select-Object -First 1; ` +
        `if ($pkg) { Add-AppxPackage -DisableDevelopmentMode -Register "$($pkg.InstallLocation)\\AppxManifest.xml" -ErrorAction SilentlyContinue; Write-Output 'reregistered' } ` +
        `else { Start-Process "ms-windows-store://pdp/?PFN=${name}"; Write-Output 'opened-store' }`;
    const code = await streamProcess(event, `appx:out:${name}`, `appx:done:${name}`,
        'powershell.exe', ['-NoProfile', '-NonInteractive', '-ExecutionPolicy', 'Bypass', '-Command', ps]);
    return { ok: code === 0, exitCode: code };
});

// --- IPC: DISM capability remove (resolve short name -> full id, then remove) -
ipcMain.handle('feature:remove', async (event, capabilityName) => {
    if (!/^[A-Za-z0-9.~_+\-]+$/.test(capabilityName || '')) return { ok: false, error: 'invalid name' };
    const out = (s) => event.sender.send(`feature:out:${capabilityName}`, s);
    // DISM needs the full capability id (e.g. "Recall~~~~0.0.1.0").
    const fullId = psQuery(
        `Get-WindowsCapability -Online | Where-Object { $_.Name -like '${capabilityName}*' -and $_.State -eq 'Installed' } | Select-Object -First 1 -ExpandProperty Name`);
    if (!fullId) {
        out('==> Capability not installed (nothing to remove).\n');
        event.sender.send(`feature:done:${capabilityName}`, { exitCode: 0 });
        return { ok: true, skipped: true, exitCode: 0, needsReboot: false };
    }
    const code = await streamProcess(event, `feature:out:${capabilityName}`, `feature:done:${capabilityName}`,
        'dism.exe', ['/Online', '/Remove-Capability', `/CapabilityName:${fullId}`, '/NoRestart']);
    return { ok: code === 0 || code === 3010, exitCode: code, needsReboot: code === 3010 };
});

// --- IPC: service optimization (reversible registry Start tweaks) -----------
// optimized vs default Start value per service. optimized:true means current
// Start >= the optimized target. Backups go to HKLM\SOFTWARE\LogPose\ServiceBackup.
const SERVICE_TARGETS = {
    SysMain:          { optimized: 3, default: 2 },
    WSearch:          { optimized: 3, default: 2 },
    Spooler:          { optimized: 3, default: 2 },
    Ndu:              { optimized: 4, default: 2 },
    Fax:              { optimized: 4, default: 3 },
    GpuEnergyDrv:     { optimized: 4, default: 3 },
    DiagTrack:        { optimized: 4, default: 2 },
    dmwappushservice: { optimized: 4, default: 3 },
};
const SVC_BACKUP = 'HKLM\\SOFTWARE\\LogPose\\ServiceBackup';

ipcMain.handle('service:list', async (event, names) => {
    const list = Array.isArray(names) ? names : Object.keys(SERVICE_TARGETS);
    const result = {};
    for (const name of list) {
        if (!/^[A-Za-z0-9]+$/.test(name)) { result[name] = { exists: false }; continue; }
        const t = SERVICE_TARGETS[name];
        const out = await new Promise((res) => {
            execFile('cmd.exe', ['/c', `reg query "${SVC}\\${name}" /v Start`], { windowsHide: true },
                (e, so) => res({ e, so: so || '' }));
        });
        if (out.e) { result[name] = { exists: false }; continue; }
        const m = out.so.match(/Start\s+REG_DWORD\s+0x([0-9a-fA-F]+)/);
        const start = m ? parseInt(m[1], 16) : null;
        const optimized = t && start != null ? start >= t.optimized : false;
        result[name] = { exists: true, start, optimized };
    }
    return result;
});

ipcMain.handle('service:set', async (event, { name, optimized }) => {
    if (!/^[A-Za-z0-9]+$/.test(name || '')) return { ok: false, error: 'invalid name' };
    const t = SERVICE_TARGETS[name];
    if (!t) return { ok: false, error: 'unknown service' };
    const reg = (args) => new Promise((res) => execFile('cmd.exe', ['/c', args], { windowsHide: true }, (e) => res(!e)));
    const queryStart = () => new Promise((res) => {
        execFile('cmd.exe', ['/c', `reg query "${SVC}\\${name}" /v Start`], { windowsHide: true },
            (e, so) => { const m = (so || '').match(/0x([0-9a-fA-F]+)/); res(m ? parseInt(m[1], 16) : null); });
    });
    if (optimized) {
        // Back up the current value once, then write the optimized Start.
        const cur = await queryStart();
        if (cur != null) {
            const has = await new Promise((res) =>
                execFile('cmd.exe', ['/c', `reg query "${SVC_BACKUP}" /v ${name}`], { windowsHide: true }, (e) => res(!e)));
            if (!has) await reg(`reg add "${SVC_BACKUP}" /v ${name} /t REG_DWORD /d ${cur} /f`);
        }
        await reg(`reg add "${SVC}\\${name}" /v Start /t REG_DWORD /d ${t.optimized} /f`);
    } else {
        // Restore from backup, or fall back to the known Windows default.
        const backup = await new Promise((res) => {
            execFile('cmd.exe', ['/c', `reg query "${SVC_BACKUP}" /v ${name}`], { windowsHide: true },
                (e, so) => { const m = (so || '').match(/0x([0-9a-fA-F]+)/); res(m ? parseInt(m[1], 16) : null); });
        });
        const restore = backup != null ? backup : t.default;
        await reg(`reg add "${SVC}\\${name}" /v Start /t REG_DWORD /d ${restore} /f`);
        await reg(`reg delete "${SVC_BACKUP}" /v ${name} /f`);
    }
    return { ok: true, needsReboot: true };
});

// --- IPC: Core Isolation / Memory Integrity (detect only) -------------------
ipcMain.handle('core-isolation:status', async () => {
    const out = psQuery(
        '(Get-CimInstance -Namespace root\\Microsoft\\Windows\\DeviceGuard -ClassName Win32_DeviceGuard).SecurityServicesRunning -join \',\'');
    return { ok: true, hvci: out.split(',').map(s => s.trim()).includes('2') };
});

// Open a Windows settings page (ms-settings: / windowsdefender:) — allowlist only.
ipcMain.handle('open-settings', async (event, uri) => {
    if (typeof uri !== 'string' || !(uri.startsWith('ms-settings:') || uri.startsWith('windowsdefender:'))) {
        return { ok: false };
    }
    try { await shell.openExternal(uri); return { ok: true }; } catch { return { ok: false }; }
});
